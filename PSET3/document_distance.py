...
def calculate_similarity_score(freq_dict1, freq_dict2):
    ...
    """
    all_keys = set(freq_dict1) | set(freq_dict2)
    diff = 0
    total = 0
...
def get_most_frequent_words(freq_dict1, freq_dict2):
    ...
    """
    all_words = set(freq_dict1) | set(freq_dict2)
    ...
    val_max = max(combined_freq.values())

    return sorted([w for w in combined_freq if combined_freq[w] == val_max])
...
def get_tf(file_path):
    ...
    * Think about how we can use get_frequencies from earlier
    ...
    dictionary = {}
    for word in freq:
        dictionary[word] = freq[word] / len(text)
    return dictionary
...
def get_idf(file_paths):
    ...
    * IDF is calculated as IDF(i) = log_10(total number of documents / number of
    ...
    for path in file_paths:
        file = load_file(path)
        words = set(get_frequencies(text_to_list(file)).keys())
...
def get_tfidf(tf_file_path, idf_file_paths):
    ...
    word_tfidf = []
    for word in tf:
        tfidf_score = tf[word] * idf.get(word, 0)
        word_tfidf.append((word, tfidf_score))
    return sorted(word_tfidf, key=lambda x: (x[1], x[0]))
