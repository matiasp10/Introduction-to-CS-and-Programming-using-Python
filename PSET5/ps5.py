...
def filter(pixels_list, color):
    ...
    for p in pixels_list:
        transformed = matrix_multiply(matrix, p)
        if isinstance(transformed, (tuple, list)):
            transformed = tuple(max(0, min(255, int(round(x)))) for x in transformed)
        else:
            transformed = tuple(max(0, min(255, int(round(transformed)))))
        pixels.append(transformed)
    return pixels
...
def extract_end_bits(num_end_bits, pixel):
    ...
    if isinstance(pixel, tuple):
        converted = []
        for i in pixel:
            converted.append(i % 2 ** num_end_bits)
...
def reveal_bw_image(filename):
    ...
    result = []
    for i in im:
        p = extract_end_bits(1, i) * 255
        result.append(p)
    return pix_to_img(result, (width, height), 'L')
...
def reveal_color_image(filename):
    ...
    result = []
    for i in im:
        p = extract_end_bits(3,i)
        result.append((p[0]*255 // 7, p[1]*255 // 7, p[2]*255 // 7))
    return pix_to_img(result, (width, height), 'RGB')
...
def main():
    ...
    pixels = img_to_pix('image_15.png')  # debe devolver una lista
    ...
    draw_kerb("Filtered image_15.png", "matiaspetenatti")

    # Uncomment the following lines to test part 2
    h1 = reveal_image('hidden1.bmp')
    h1.show()
    h1.save("Unhidden hidden1.bmp")

    h2 = reveal_image('hidden2.bmp')
    h2.show()
    h2.save("Unhidden hidden2.bmp")

    draw_kerb("Unhidden hidden1.bmp", "matiaspetenatti")
    draw_kerb("Unhidden hidden2.bmp", "matiaspetenatti")
